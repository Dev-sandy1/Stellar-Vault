 ...
 * browser package today, so Stellar-Vault evaluates squads with the on-device engine in
...
export async function enrichReasoningWithQvac(
  ...
  try {
    const text = await adapter.complete({
      system:
        'You are Stellar-Vault, an on-device fantasy football agent. Write formal British English. No em dashes. Be concise.',
